...
let aliens = [];
let lasers = [];
let spaceshipObstacles = []; // Stacked spaceships instead of pipes
let buildings = [];
let snow = [];
let planets = [];
let thunders = [];
...
function initWorld() {
    ...
    // Create static white buildings in background
...
function update() {
    ...
    if (frameCount % 60 === 0) time++; // Increment time every second
    
    // Spawn thunder occasionally
    ...
    // Spawn STACKED SPACESHIPS (instead of pipes)
    if (frameCount % config.obstacleSpawnRate === 0) {
        const gapSize = 200; // Good gap for flying through
        const gapStart = 100 + Math.random() * (canvas.height - gapSize - 200);
        
        // Create top stack of spaceships
        const topShips = [];
        const numTopShips = Math.floor(gapStart / 45); // Stack spaceships vertically
    ...
    // Update spaceship obstacles
    spaceshipObstacles.forEach((obstacle, i) => {
        obstacle.x -= config.speed;
        
        // Check collision with stacked spaceships
        obstacle.topShips.forEach(ship => {
            if (eagle.x + eagle.w > obstacle.x - 20 && eagle.x < obstacle.x + 40 &&
        ...
        if (obstacle.x < -100) {
            spaceshipObstacles.splice(i, 1);
        }
    });
    
    // Spawn enemies - moving dragons and swords (bigger objects)
    if (frameCount % config.enemySpawnRate === 0) {
        const enemyType = Math.random();
        const yPos = 80 + Math.random() * (canvas.height - 180);
        
        if (enemyType < 0.5) {
            // Spawn moving dragon (bigger)
            dragons.push({ 
                x: canvas.width, 
                y: yPos, 
                w: 55, 
            ...
            // Spawn moving rotating sword (bigger)
            swords.push({
                x: canvas.width,
                y: yPos,
                w: 50,
    ...
    // Spawn alien allies occasionally
    if (frameCount % 180 === 0) {
        aliens.push({
            x: canvas.width,
    ...
    // Update moving dragons
    dragons.forEach((d, i) => {
        d.x -= config.speed;
        d.wingFlap += 0.15;
        
        // Add wave motion (moving up and down)
        d.waveOffset += d.waveSpeed;
        d.y += Math.sin(d.waveOffset) * 1.5;
        
        // Dragon shoots flames
    ...
    // Update moving rotating swords
    swords.forEach((s, i) => {
        s.x -= config.speed;
        s.rotation += s.rotationSpeed;
        
        // Add bobbing motion (moving up and down)
        s.bobOffset += s.bobSpeed;
        s.y += Math.sin(s.bobOffset) * 1.2;
        
        // Check collision
        if (eagle.x < s.x + s.w && eagle.x + eagle.w > s.x && 
            eagle.y < s.y + s.h && eagle.y + eagle.h > s.y) {
            if (eagle.isAttacking) {
                swords.splice(i, 1);
                playEagleScreech();
            } else {
    ...
    aliens.forEach((alien, i) => {
        alien.x -= config.speed * 0.8;
        alien.shootTimer--;
        
        // Alien shoots red laser
    ...
    lasers.forEach((laser, i) => {
        ...
        if (eagle.x < laser.x + laser.width && eagle.x + eagle.w > laser.x &&
            eagle.y < laser.y + laser.height && eagle.y + eagle.h > laser.y) {
            gameOver();
        }
...
function draw() {
    ...
    // Draw thunder bolts (no screen flash)
    thunders.forEach(t => {
    ...
    // Draw static white buildings in background
    ctx.globalAlpha = 0.5;
    buildings.forEach(b => {
    ...
    // Draw moving dragons (bigger)
    dragons.forEach(d => {
        // Dragon body with wing animation
        ctx.save();
    ...
    // Draw moving rotating red swords (bigger)
    swords.forEach(s => {
        ctx.save();
        ctx.translate(s.x + 25, s.y + 25);
        ctx.rotate(s.rotation);
        
        // Draw red sword with glow
        ctx.shadowBlur = 10;
    ...
    // Draw alien allies (typical alien character with black eyes)
    aliens.forEach(alien => {
        ctx.font = "40px serif";
        ctx.fillText("👽", alien.x, alien.y + 30);
    });
